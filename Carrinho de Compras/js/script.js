...
modelsJson.map((item, index)=>{
    ...
    modelsItem.querySelector('.models-item--nome').innerHTML = item.name;
...
//Açoes do modalv-janelas
      ...
      cs('.modelsInfo--cancelButtom, .modelsInfo--cencelMobilebutton').forEach((item)=>{
        item.addEventListener('click', closeModal);
      }); 

      c('.modelsIfo--qtmenos').addEventListener('click', () => {
        if(modalQT > 1){
        modalQT--;
        c('.modelsInfo--qt').innerHTML = modalQT;
        }
      ...
      c('.modelsInfo--addButtom').addEventListener('click', ()=>{
        let size =  parseInt(c('.modelsInfo--size.selected').getAttribute('data-key'));
        let identifier = modelsJson[key].id+'@'+size;
        let locald = cart.findIndex((item)=>item.identifier == identifier);
        if(locald > -1){
          cart[locald].qt += modalQT;
        }else{
            cart.push({
            identifier,
      ...
      function  updateCart(){
        ...
        if(cart.length > 0){
          ...
          cart.map((itemCart, index)=>{
            ...
            subtotal += modelsItem.price[itemCart.size] * itemCart.qt;
            ...
            //cartItem.querySelector('.cart--item-nome').innerHTML = `${modelsItem.name} (${modelsSizeName})`;
            cartItem.querySelector('.cart--item-nome').innerHTML = `${modelsItem.name} ${modelsItem.sizes[itemCart.size]}`; //Para eu colocar em g,m e p basta colocar ${modelsItem.name} (${modelsSizeName})`
